...
class AmharicNERLabeler:
    ...
    def label_dataframe(self, df, token_column):
        ...
        """
        # Ensure tokens in the column are lists
        if not all(isinstance(entry, list) for entry in df[token_column]):
            raise ValueError(f"The '{token_column}' column must contain lists of tokens.")
